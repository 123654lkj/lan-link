...
enum Cmd {
    ...
    Batch { file: String, #[arg(long, default_value = "10")] timeout: u64 },
    ...
    Pull { #[arg(short)] remote: String, #[arg(short)] local: String },
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Cmd::Batch { file, timeout } => {
            ...
            println!("Batch complete: {} ok, {} failed ", ok, fail);
        }
                ...
                match ctx.recv_control(rem.min(Duration::from_secs(2))).await {
                    Ok(Some(msg)) => match msg {
                        ...
                        ControlMsg::ExecDone { id: eid, .. } if eid == id => {
                            ...
                            eprintln!("Pulled {} bytes -> {}", dlen, local);
                            return Ok(());
                        }
                        _ => {}
                    }
...
async fn cmd_push(ctx: &mut Ctx, local: &str, remote: &str) -> anyhow::Result<()> {
    ...
    info!("Push complete: {} bytes ", data.len());
    Ok(())
}
