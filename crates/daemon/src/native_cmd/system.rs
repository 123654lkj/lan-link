...
pub fn cmd_who() -> (Vec<u8>, Option<i32>) {
    match Command::new("/usr/bin/who").output() {
...
pub fn cmd_kill(pid: u32, signal: u32) -> (Vec<u8>, Option<i32>) {
    match Command::new("/usr/bin/kill")
        .arg(format!("-{}", signal))
        .arg(format!("{}", pid))
        .output()
    {
...
pub fn cmd_ssh_check() -> (Vec<u8>, Option<i32>) {
    match Command::new("/usr/bin/ssh").arg("-V").output() {
        Ok(o) => {
...
pub fn cmd_last(lines: u32) -> (Vec<u8>, Option<i32>) {
    let mut cmd = Command::new("/usr/bin/last");
