...
impl Connection {
    ...
    pub fn build_syn(conn_id: u64) -> BytesMut {
        ...
        PacketHeader {
            conn_id, pkt_type: PacketType::Syn, flags: Flags::empty(),
            stream_id: StreamId::Control as u16, seq: 0, ack_seq: 0,
            ack_bitmap: 0, payload_len: 0, nonce: [0u8; 12],
        }.encode(&mut buf);
        buf
    }
    ...
    pub fn build_syn_ack(conn_id: u64) -> BytesMut {
        let mut buf = BytesMut::with_capacity(HEADER_SIZE);
        PacketHeader {
            conn_id, pkt_type: PacketType::SynAck, flags: Flags::empty(),
            stream_id: StreamId::Control as u16, seq: 0, ack_seq: 0,
            ack_bitmap: 0, payload_len: 0, nonce: [0u8; 12],
        }.encode(&mut buf);
        buf
    ...
    pub fn build_data(conn_id: u64, stream_id: u16, seq: u32, flags: Flags, payload: &[u8]) -> BytesMut {
        ...
        PacketHeader {
            conn_id, pkt_type: PacketType::Data, flags, stream_id, seq,
    ...
    pub fn build_heartbeat(conn_id: u64) -> BytesMut {
        let mut buf = BytesMut::with_capacity(HEADER_SIZE);
        PacketHeader {
            conn_id, pkt_type: PacketType::Heartbeat, flags: Flags::empty(),
            stream_id: StreamId::Control as u16, seq: 0, ack_seq: 0,
            ack_bitmap: 0, payload_len: 0, nonce: [0u8; 12],
        }.encode(&mut buf);
        buf
