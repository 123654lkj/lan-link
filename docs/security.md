...
> ⚠️ **⚠️ 核心安全警告 ⚠️**
>
> **持有 PSK = 完整远程代码执行（RCE）权限。**
>
> `ControlMsg::Exec` 和 `NativeCmdType::ShellExec` 允许在 daemon 主机上执行**任意 shell 命令**。这是**设计意图**，不是缺陷。结构化命令（`NativeCmdType::Fs`、`System` 等）是 API 层面的便利封装，但 `ShellExec` 和 `Exec` 始终存在。
>
> **⛔ 绝不可将 daemon 端口（默认 9876）暴露到公网或不可信网络。**
> **⛔ 必须配合 VPN/WireGuard 隧道使用。**
> **⛔ PSK 泄露等同于主机完全沦陷。**
...
- **已实现**：daemon 内置滑动窗口速率限制器（纯 Rust，无外部依赖）
- **SYN 限速**：每个源 IP 每 60 秒最多 5 次 SYN 握手
- **命令限速**：每个源 IP 每 60 秒最多 30 条控制命令（解密成功后计数）
- **连接数上限**：单个 daemon 最多 100 个并发连接
- **内存保护**：每 heartbeat tick 自动 GC 过期条目，防止内存无限增长
- **额外建议**：仍可在网络层（iptables/nftables）增加纵深防御
